...
**Bipolar I disorder with comorbid generalized anxiety disorder.** First-line GAD
treatment is antidepressants; antidepressant monotherapy in bipolar I risks manic switch
and cycle acceleration. The field's answer has been to reach for whatever mood
stabiliser also happens to be sedating. That is a workaround, not a solution.

The real question underneath is mechanistic:

> **What decouples anxiolysis from mood destabilisation?**

The two halves are not the same pathway. Something separates them. Find candidates that
...
Here, **novelty is the point**. Do not propose the first-line option, and do not propose
"add one more standard agent". If a psychiatrist would already have thought of it in the
first thirty seconds, it does not belong here.

Reach for:

- **mechanistic targets nobody has connected to this population** — the map below is a
starting point, not a menu - **drugs approved for something else entirely** whose
mechanism happens to fit - **compounds in phase 1-2 for other indications** -
**non-pharmacological interventions** that act on the same target: neuromodulation,
timing-based, metabolic, physiological, device-based - **timing and sequencing** as the
intervention itself, rather than the molecule - **the inverse question** — what makes
the standard option harmful here, and can that specific property be subtracted while
keeping the benefit?
...
right mechanism but the wrong side-effect profile. Naming a compound that has never been
given to a patient in this population is fine — that is the point of this track.
...
sparing the benefit? Name the components and say what the pairing does that neither does
alone.

**Non-oral routes and devices.** `route` is a mechanistic variable, not packaging.
Switch risk tracks the *rate* of monoaminergic rise, and route sets Tmax — a transdermal
patch or subcutaneous depot flattens a curve the same molecule spikes when swallowed. So
the same compound may be unusable orally and viable transdermally, and that is a real
hypothesis.
...
population. Be honest: inflating it wastes the reviewer's attention, and the ranking is
plausibility × novelty, so a dishonest 1.0 on a weak chain still ranks poorly.

`known_risks` — what could go wrong in the target population specifically. Empty only if
genuinely nothing is known, which is itself worth stating.
