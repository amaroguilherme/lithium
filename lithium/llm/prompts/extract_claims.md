...
and true when it does. When it is false, `directness` is ignored entirely, so do not
try to pick a safe level: guessing the weakest one is not caution here, it is a wrong
