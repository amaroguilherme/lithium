...
that a $reader will read **with the citations in front of them**? That is the whole
...
adjacent one? $sufficiency_population_rule Be strict here; it is the field that
separates a real answer from a plausible one.
...
`missing`, and every one of them is compatible with `sufficient: true`. They describe the
literature, and the note is allowed to say so.
