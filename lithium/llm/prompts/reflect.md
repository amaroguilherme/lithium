...
taken from the claims block below. If any index you give was not shown to you, the
whole lesson is thrown away — not just that index.
...
- restatements of what the corpus already holds as claims
- one-off events with no generalisation ("this fetch timed out")
- anything you could not act on next time
...
`claim_ids` — required for `kind: "pattern"`, empty for process lessons. Give the
`[k]` indices from the claims block, nothing else.
...
- `claim_ids` takes `[k]` indices, and only ones from the claims block. An index that
  ...
  discarded and the mistake is logged.
- Do not put `[k]` inside `text`. The lesson has to read correctly on its own.
